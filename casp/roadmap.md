...
|------|--------|-------|-------|
| 2026-09-10 | `pending` | **0.18.0** — the schedule layer : opt-in `casp/schedule.json`, four `CASP-SCHEDULE-*` rules, the `casp schedule` verb, the board in `status` and `close` | 231 → 256 tests; the clock may add a WARN and may never add a FAIL, pinned by two tests; **not yet published to npm** |
